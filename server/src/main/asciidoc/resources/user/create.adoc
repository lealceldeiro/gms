...
This service is only available to users with administrative permissions to register new users directly without going through the signing up process.
...
A `POST` request adds a new user.
This service is only available if the system is configured to accept new users via signing up process.
